...
- "/duration" - метол POST устанавливает нужное время (в секундах) для каждой арифметической операции для каждого пользователя. Метод GET вернет текущие настройки времени. Если настроек нет - вернет значения по умолчанию. С авторизацией

- "/expression" - метод POST принимает нужное выражение. Поддерживаются операции "+", "-", "*", "/", а так же скобки "(" и ")". Все пробелы игнорируются (удаляются). Если выражение корректное - начнется высчитываться выражение и вернет выражение с его ID, датой его создания, датой завершения и его текущий статус. Если некорректное - вернет тоже самое выражение, но со статусом error. Если написать выражение, которое уже было, то вернется это выражение, его ID и статус. С авторизацией

- "/getexpressions" - метод GET возвращает все выражения, которые пользователь ввел ранее. Если выражений нет - вернет пустой список

- "/id/{id}" - метод POST принимает ID выражения и возвращает его, если оно найдено. Если нет - возвращается статс 400 и ошибка.
